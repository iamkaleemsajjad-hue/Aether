...
class TorchEncoderAEGEngine:
    ...
    def __init__(self, source_engine: Any, device: str) -> None:
        ...
        for layer in self.weights.layers:
            self.layers.append({
                name: self._tensor(getattr(layer, name))
                for name in (
                    "q_proj", "k_proj", "v_proj", "o_proj", "attention_norm",
                    "intermediate_proj", "output_proj", "output_norm",
                )
            } | {
                f"{name}_bias": self._optional(getattr(layer, f"{name}_bias", None))
                for name in (
    ...
    def _optional(self, value: Any | None) -> Any | None:
    ...
    def encode(
        ...
        return (hidden * mask[:, :, None]).float().cpu().numpy()
...
class TorchSeq2SeqAEGEngine:
    ...
    def __init__(self, source_engine: Any, device: str) -> None:
        ...
        self.encoder_layers = [self._convert_layer(layer) for layer in source_engine.encoder_layers]
        self.decoder_layers = [self._convert_layer(layer) for layer in source_engine.decoder_layers]
    ...
    def _convert_layer(self, layer: Any) -> dict[str, Any | None]:
        result = {name: self._tensor(getattr(layer, name)) for name in layer.__dataclass_fields__ if getattr(layer, name) is not None}
        for name in ("relative_bias", "ffn_in_0", "ffn_in_1"):
            result[name] = self._optional(getattr(layer, name, None))
        return result
    ...
    def _relative_bucket(self, relative: Any, bidirectional: bool) -> Any:
        ...
        large = max_exact + (
            self.torch.log(self.torch.maximum(relative, self.torch.tensor(max_exact, device=self.device, dtype=relative.dtype)) / max_exact)
    ...
    def _attention(self, q: Any, k: Any, v: Any, query_positions: Any, key_positions: Any, relative_bias: Any | None, bidirectional: bool) -> Any:
        ...
        allowed = torch.ones((query_positions.numel(), key_positions.numel()), dtype=torch.bool, device=self.device)
        ...
        if relative_bias is not None and relative_bias.ndim == 2:
            ...
            bias = torch.zeros((self.num_heads, query_positions.numel(), key_positions.numel()), device=self.device)
    ...
    def generate(self, prompt_ids: Any, max_tokens: int = 16, temperature: float = 0.0, top_k: int = 0, top_p: float = 1.0, eos_token_id: int | None = None, **_: Any) -> list[int]:
        if max_tokens < 1:
